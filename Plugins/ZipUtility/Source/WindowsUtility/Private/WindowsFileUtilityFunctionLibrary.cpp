...
TMap<FString, TArray<FWatcher>> FWindowsFileUtilityFunctionLibrary::Watchers = TMap<FString, TArray<FWatcher>>();
int TotalWatchers = 0;
...
bool FWindowsFileUtilityFunctionLibrary::DoesFileExist(const FString& FullPath)
{
	return 0 != PathFileExistsW(*FullPath);
}

bool FWindowsFileUtilityFunctionLibrary::MoveFileTo(const FString& From, const FString& To)
{
	//Using windows api
...
bool FWindowsFileUtilityFunctionLibrary::CreateDirectoryAt(const FString& FullPath)
{
	//Using windows api
	return 0 != CreateDirectoryW(*FullPath, NULL);
}

bool FWindowsFileUtilityFunctionLibrary::DeleteFileAt(const FString& FullPath)
{
	//Using windows api
	return 0 != DeleteFileW(*FullPath);
}

bool FWindowsFileUtilityFunctionLibrary::DeleteEmptyFolder(const FString& FullPath)
{
	//Using windows api
...
bool FWindowsFileUtilityFunctionLibrary::DeleteFolderRecursively(const FString& FullPath)
{
...
void FWindowsFileUtilityFunctionLibrary::WatchFolder(const FString& FullPath, TSharedPtr<IWFUFolderWatchInterface,ESPMode::ThreadSafe> WatcherDelegate)
{
	...
	FreshWatcher.ThreadFuture = FWFULambdaRunnable::RunLambdaOnBackGroundThread([FullPath, WatcherDelegate, WatcherPtr]()
	{
		 FWindowsFileUtilityFunctionLibrary::WatchFolderOnBgThread(FullPath, WatcherPtr);
	});
...
void FWindowsFileUtilityFunctionLibrary::StopWatchingFolder(const FString& FullPath, TSharedPtr<IWFUFolderWatchInterface, ESPMode::ThreadSafe> WatcherDelegate)
{
	//Do we have an entry?
	if (!Watchers.Contains(FullPath))
	{
...
void FWindowsFileUtilityFunctionLibrary::ListContentsOfFolder(const FString& FullPath, TSharedPtr<IWFUFileListInterface,ESPMode::ThreadSafe> Delegate)
{
	...
	FWFULambdaRunnable::RunLambdaOnBackGroundThread([&FullPath, Delegate]()
	{
		WIN32_FIND_DATA ffd;
		...
		do
		{
			...
			else if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				...
				FWFULambdaRunnable::RunShortLambdaOnGameThread([Delegate, ItemPath, Name]
				{
					Delegate->OnListDirectoryFound(Name, ItemPath);
				});
			}
			//File
			else
			{
				...
				FWFULambdaRunnable::RunShortLambdaOnGameThread([Delegate, ItemPath, Name, TruncatedFileSize]
				{
					Delegate->OnListFileFound( Name, TruncatedFileSize, ItemPath);
				});
			}
		...
		FWFULambdaRunnable::RunShortLambdaOnGameThread([Delegate, FullPath, FileNames, FolderNames]
		{
			Delegate->OnListDone(FullPath, FileNames, FolderNames);
		});
	});
}

void FWindowsFileUtilityFunctionLibrary::ListContentsOfFolderToCallback(const FString& FullPath, TFunction<void(const TArray<FString>&, const TArray<FString>&)> OnListCompleteCallback)
{
	
	TSharedPtr<FWFUFileListLambdaDelegate, ESPMode::ThreadSafe> LambdaDelegate = MakeShareable(new FWFUFileListLambdaDelegate);
	LambdaDelegate->SetOnDoneCallback(OnListCompleteCallback);
	ListContentsOfFolder(FullPath, LambdaDelegate);
}

void FWindowsFileUtilityFunctionLibrary::WatchFolderOnBgThread(const FString& FullPath, const FWatcher* WatcherPtr)
{
	...
	const TSharedPtr<IAbstructInterface,ESPMode::ThreadSafe> WatcherDelegate = WatcherPtr->Delegate;
	...
	while (WatcherPtr->ShouldRun)	//Watcher.Runnable->Finished == false
	{
		...
		case WAIT_OBJECT_0:
			...
			FWFULambdaRunnable::RunShortLambdaOnGameThread([FullPath, FileString, WatcherDelegate]()
			{
				if (WatcherDelegate->GetType() == EDelegateInterfaceType::WATCH_INTERFACE)
				{
					FString FilePath = FString::Printf(TEXT("%s\\%s"), *FullPath, *FileString);
					StaticCastSharedPtr<IWFUFolderWatchInterface, IAbstructInterface,ESPMode::ThreadSafe>(WatcherDelegate)->OnFileChanged(FileString, FilePath);
				}
			});
		...
		case WAIT_OBJECT_0 + 1:
			...
			FWFULambdaRunnable::RunShortLambdaOnGameThread([FullPath, WatcherDelegate, DirectoryString]()
			{
				if (WatcherDelegate->GetType() == EDelegateInterfaceType::WATCH_INTERFACE)
				{
					FString ChangedDirectoryPath = FString::Printf(TEXT("%s\\%s"), *FullPath, *DirectoryString);					
					StaticCastSharedPtr<IWFUFolderWatchInterface, IAbstructInterface, ESPMode::ThreadSafe>(WatcherDelegate)->OnDirectoryChanged(DirectoryString, ChangedDirectoryPath);
				}
			});
