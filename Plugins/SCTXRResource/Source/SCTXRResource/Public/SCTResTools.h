 ...
 * 
 */
class SCTXRRESOURCE_API FSCTResTools
{
public:
	FSCTResTools();
	...
	/**
	 * @brief 通过文件名来构建hash值，可以通过该hash值来生成ID
	 * @param[in] _FileName 文件名 
	 * @return 返回hash值
	 * @note 2018年8月7日 欧石楠 添加，由于已有的接口GetFileIDFromName对传入的文件名限制比较严格(文件名前面必须包含md5码，并且使用_来与后面做分割)
	 */
	static uint32 GetFileIDFromNameHashValue(FString _FileName);
	...
	bool XROpenFileDialog(const FString& DialogTitle, const FString& DefaultPath, TArray<FString>& OutFileNames, FString _FileType, bool _bMulty);
