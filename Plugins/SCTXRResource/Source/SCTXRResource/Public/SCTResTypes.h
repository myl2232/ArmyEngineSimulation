...
#include "SCTResTypes.generated.h"

struct FSCTMeshMaterialData
{
	FString SlotName;
	int32 PakID;
	FString ParaStr;
	...
	FSCTMeshMaterialData() {}
	FSCTMeshMaterialData(FString _SlotName, int32 _PakID, FString _ParaStr,
...
struct FSCTXRParameter
{
	...
	void OverrideValue(FSCTXRParameter& _Src)
	{
	...
	FSCTXRParameter(FString _DisplayName, FString _DisplayNameEN, FString _ParaName, float _MinValue, float _MaxValue, float _Value = 0.f)
	{
		ID = 0;
	...
	FSCTXRParameter(FString _DisplayName, FString _DisplayNameEN, FString _ParaName, FVector _Color = FVector::ZeroVector)
	{
		ID = 0;
		ParaDisplayName = _DisplayName;
		ParaDisplayNameEN = _DisplayNameEN;
	...
	FSCTXRParameter(FString _DisplayName, FString _DisplayNameEN, FString _ParaName, bool _bCheck)
	{
		ID = 0;
		ParaDisplayName = _DisplayName;
		ParaDisplayNameEN = _DisplayNameEN;
...
struct FSCTCommonMaterialData
{
	...
	FSCTCommonMaterialData() {}
	FSCTCommonMaterialData(FString _MaterialName, FString _MaterialDisplayName, TArray<int32> _ExposedParameters)
...
//模型外接信息
USTRUCT()
struct FSCTModelMaterialData
{
	GENERATED_USTRUCT_BODY()
		FString SlotName;
	//V7材质球类型
	FSCTCommonMaterialData::EMaterialType MaterialType;
	//正在使用的动态材质
	UPROPERTY()
	...
	FSCTModelMaterialData(FString _SlotName, FSCTCommonMaterialData::EMaterialType _MaterialType, UMaterialInstanceDynamic* _DIM, TArray<FSCTXRParameter>& _ParaList, bool _bReplaceable = false)
	...
	void ResetToServer()
	{
		...
		{
			It.ResetToServer();
			if (It.VarType == FSCTXRParameter::EVarType::E_Float)
			{
				DIM->SetScalarParameterValue(FName(*It.ParaName), It.Value);
			}
			else if (It.VarType == FSCTXRParameter::EVarType::E_Vec3)
			{
