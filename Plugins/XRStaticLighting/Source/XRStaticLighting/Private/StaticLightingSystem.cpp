...
void FStaticLightingManager::BuildStaticLighting(int32 InQuality)
{
	bMapBuildSuccessfully = false;

	SetMapBuildCancelled(false);
	BuildPercent = 0.f;

	for (TObjectIterator<ULightComponent> ObjItr; ObjItr; ++ObjItr)
	{
		if (!(ObjItr->GetFlags() & RF_ArchetypeObject))
			(*ObjItr)->InvalidateLightingCacheInner(true);
	}

	StopRenderingThread();
	FLightingBuildOptions Options;
	Options.QualityLevel = (ELightingBuildQuality)InQuality;
	CreateStaticLightingSystem(Options);
	StartRenderingThread();
}

void FStaticLightingManager::ClearStaticLighting()
{
	bMapBuildLoaded = false;
	if (GWorld->GetCurrentLevel()->MapBuildData)
	{
		bMapBuildSuccessfully = false;
		GWorld->GetCurrentLevel()->MapBuildData->InvalidateStaticLighting(GWorld);
		GWorld->GetCurrentLevel()->MapBuildData->ConditionalBeginDestroy();
		GWorld->GetCurrentLevel()->MapBuildData = NULL;
	}
}
...
void FStaticLightingManager::ProcessLightingData()
{
	...
	{
		FStaticLightingManager::Get()->bMapBuildSuccessfully = true;
	}
...
void FStaticLightingManager::SendBuildDoneNotification( bool AutoApplyFailed )
{
	...
	BuildStage = EBuildStage::ESuccess;
}
...
void FStaticLightingManager::UpdateBuildLighting()
{
	...
	{
		if(ActiveStaticLightingSystem->GetBuildStage() == 2)
			BuildStage = EBuildStage::EPending;
		else
			BuildStage = EBuildStage::EProcessing;
		...
		if (ActiveStaticLightingSystem)
			BuildPercent = ActiveStaticLightingSystem->GetBuildPercent();
...
// 	else
	...
	BuildStage = EBuildStage::EFailed;
}
...
void FStaticLightingManager::GatherStaticLightingInfo()
{
	if (!ActiveStaticLightingSystem)
	{
		FLightingBuildOptions Options;
		...
		ActiveStaticLightingSystem = StaticLightingSystems[0].Get();
		ActiveStaticLightingSystem->GatherStaticLightingInfo(false, false);

		DestroyStaticLightingSystems();
	}
}

bool FStaticLightingManager::LoadMapBuildData(const FString InRelativePackagePath)
{
	ClearStaticLighting();
	bool LoadResult = false;
	if (IPlatformFile::GetPlatformPhysical().FileExists(*InRelativePackagePath))
	{
		GatherStaticLightingInfo();
		...
		FString LongPackageName = FPackageName::FilenameToLongPackageName(InRelativePackagePath);
		
		UObject* LastPackage = StaticFindObjectFast(UPackage::StaticClass(),NULL, FName(*LongPackageName));
		if (LastPackage)
		{
			LastPackage->ConditionalBeginDestroy();
			LastPackage = NULL;
		}

		UPackage* ResultPackage = LoadPackageFromRuntime(NULL, *LongPackageName, RF_NoFlags);
		...
		if (ResultObj)
		{
			...
			{
				ULevel* TheLevel = GWorld->GetCurrentLevel();
...
bool FStaticLightingManager::SaveMapBuildData(const FString InFilePath)
{
	bool SaveResult = false;

	//如果数据为空，可能是没有经过任何烘焙计算，也可能是刚刚清除了光照数据，此时保存方案，应该尝试删除已有的烘焙数据包文件
	if (GWorld->GetCurrentLevel()->MapBuildData == NULL)
		return false;
	
	//打开方案，刚刚成功执行过光照计算后，点保存才会执行数据包文件的写入
	if (!bMapBuildSuccessfully)
		return false;

	IPlatformFile* OldPlatformFile = &FPlatformFileManager::Get().GetPlatformFile();
	FPlatformFileManager::Get().SetPlatformFile(IPlatformFile::GetPlatformPhysical());
	UPackage* SavePackage = NULL;

	UObject* Obj = StaticFindObject(UPackage::StaticClass(), NULL, TEXT("/Game/XRCommon/Map/DIYTemplate_Grass_BuiltData"));
	if (!Obj)
	{
		FString LongPackageName = FPackageName::FilenameToLongPackageName(InFilePath);
		//FString LoadPath = FString::Printf(TEXT("/Game/DBJCache/LevelPlan/%d_LevelData"), FArmyUser::Get().GetCurPlanID());
		Obj = StaticFindObject(UPackage::StaticClass(), NULL, *LongPackageName);
	}

	SavePackage = Cast<UPackage>(Obj);

	if (SavePackage)
	{
		SavePackage->SetPackageFlags(PKG_FilterEditorOnly);
		//FString SavePath = FString::Printf(TEXT("%s%d_LevelData.uasset"), *FResTools::GetLevelPlanDir(), FArmyUser::Get().GetCurPlanID());
		const FSavePackageResultStruct Result = UPackage::Save(SavePackage, NULL, RF_Standalone, *InFilePath);
...
{
	extern XRSTATICLIGHTING_API bool GLightmassStatsMode;
...
void FStaticLightingSystem::GatherScene()
{
	...
	// Meshes
	//@打扮家 XRLightmass
	for (int32 MeshIdx = 0; !FStaticLightingManager::Get()->GetMapBuildCancelled() && MeshIdx < Meshes.Num(); MeshIdx++)
	{
		if (Meshes[MeshIdx]->bCustomMesh)
			Meshes[MeshIdx]->ExportMeshInstance(LightmassExporter);
		else
		{
			FStaticMeshStaticLightingMesh* Tmp = (FStaticMeshStaticLightingMesh*)(Meshes[MeshIdx].GetReference());
			if (Tmp)
			{
				LightmassExporter->StaticMeshLightingMeshes.AddUnique(Tmp);

				for (int32 LightIdx = 0; LightIdx < Tmp->RelevantLights.Num(); LightIdx++)
				{
					ULightComponent* Light = Tmp->RelevantLights[LightIdx];
					if (Light)
					{
						LightmassExporter->AddLight(Light);
					}
				}

				// Add the UStaticMesh and materials to the exporter...
				if (Tmp->StaticMesh && Tmp->StaticMesh->RenderData)
				{
					LightmassExporter->StaticMeshes.AddUnique(Tmp->StaticMesh);
					if (Tmp->Primitive)
					{
						for (int32 ResourceIndex = 0; ResourceIndex < Tmp->StaticMesh->RenderData->LODResources.Num(); ++ResourceIndex)
						{
							const FStaticMeshLODResources& LODResourceData = Tmp->StaticMesh->RenderData->LODResources[ResourceIndex];
							for (int32 SectionIndex = 0; SectionIndex < LODResourceData.Sections.Num(); ++SectionIndex)
							{
								...
								UMaterialInterface* Material = Tmp->Primitive->GetMaterial(Section.MaterialIndex);
								LightmassExporter->AddMaterial(Material);
							}
						}
					}
				}
	...
	// Mappings
	//@打扮家 XRLightmass
	for (int32 MappingIdx = 0; !FStaticLightingManager::Get()->GetMapBuildCancelled() && MappingIdx < Mappings.Num(); MappingIdx++)
	{
		if (Mappings[MappingIdx]->bCustomMapping)
			Mappings[MappingIdx]->ExportMapping(LightmassExporter);
		else
		{
			FStaticMeshStaticLightingTextureMapping* Tmp = (FStaticMeshStaticLightingTextureMapping*)(Mappings[MappingIdx]->GetTextureMapping());
			if(Tmp)
				LightmassExporter->StaticMeshTextureMappings.AddUnique(Tmp);
		}
...
bool FStaticLightingSystem::InitiateLightmassProcessor()
{
	...
	//@打扮家 XRLightmass
	if (!FStaticLightingManager::Get()->GetMapBuildCancelled())
	{
...
bool FStaticLightingSystem::FinishLightmassProcess()
{
	...
	
	{
...

		{
			...
			//@打扮家 XRLightmass
			bBuildCanceled = bBuildCanceled || FStaticLightingManager::Get()->GetMapBuildCancelled();
...
void FStaticLightingSystem::UpdateLightingBuild()
{
	...
	if (CurrentBuildStage == FStaticLightingSystem::AmortizedExport)
	{
		...
		BuildPercent = LightmassProcessor->GetAmortizedExportPercentDone() * 100.f;
	...
	else if (CurrentBuildStage == FStaticLightingSystem::AsynchronousBuilding)
	{
		...
		BuildPercent = LightmassProcessor->GetAsyncPercentDone();
	...
	{
		//@打扮家 XRLightmass
		if ( true /*CanAutoApplyLighting()*/ || IsRunningCommandlet() )
		{
...
{
	const bool bAutoApplyEnabled = false;// GetDefault<ULevelEditorMiscSettings>()->bAutoApplyLightingEnable;
	const bool bSlowTask = GIsSlowTask;
	const bool bInterpEditMode = false;// GLevelEditorModeTools().IsModeActive(FBuiltinEditorModes::EM_InterpEdit);
	const bool bPlayWorldValid = true;// GEditor->PlayWorld != nullptr;
