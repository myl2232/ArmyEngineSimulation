...
//@打扮家 XRLightmass 增加 : public FTickableGameObject
class FStaticLightingManager : public FTickableGameObject, public TSharedFromThis<FStaticLightingManager>
{
public:
	XRSTATICLIGHTING_API static TSharedPtr<FStaticLightingManager> Get();

	XRSTATICLIGHTING_API void BuildStaticLighting(int32 InQuality);

	XRSTATICLIGHTING_API void ClearStaticLighting();

	XRSTATICLIGHTING_API void SetMapBuildCancelled(bool bCancel) { bCancelBuild = bCancel; }

	XRSTATICLIGHTING_API bool GetMapBuildCancelled() { return bCancelBuild; }

	XRSTATICLIGHTING_API void GatherStaticLightingInfo();

	XRSTATICLIGHTING_API bool LoadMapBuildData(const FString InRelativePackagePath);

	XRSTATICLIGHTING_API bool SaveMapBuildData(const FString InFilePath);

	XRSTATICLIGHTING_API bool IsValidLightMap() { return bMapBuildSuccessfully || bMapBuildLoaded; }

	//@打扮家 XRLightmass 增加了自主Tick的功能，在非Editor模式下，需要驱动UpdateBuildLighting的执行
	virtual bool IsTickable() const override { return true; }
	virtual bool IsTickableWhenPaused() const override { return true; }
	virtual TStatId GetStatId() const override { RETURN_QUICK_DECLARE_CYCLE_STAT(FStaticLightingManager, STATGROUP_Tickables); }
	virtual void Tick(float DeltaTime) override { UpdateBuildLighting(); }
	...
	XRSTATICLIGHTING_API void CreateStaticLightingSystem(const FLightingBuildOptions& Options);
	...
	EBuildStage GetBuildStage() { return BuildStage; }
	float GetBuildPercent() { return BuildPercent; }

private:
	FStaticLightingManager() 
		: ActiveStaticLightingSystem(NULL) 
		, BuildStage(EBuildStage::ENotStart)
		, BuildPercent(0.f)
		, bCancelBuild(false)
	...
	EBuildStage BuildStage;
	float BuildPercent;
	bool bCancelBuild;
	//表示已经成功构建了光照，此时保存有效
	bool bMapBuildSuccessfully = false;
	//标识刚刚加载过了光照文件
	bool bMapBuildLoaded = false;
};
...
private:
	...
	int32 GetBuildStage() { return CurrentBuildStage; }
	float GetBuildPercent() { return BuildPercent; }
...
//XRSTATICLIGHTING_API extern FSelectedLightmapSample GCurrentSelectedLightmapSample;

/** Information about the last static lighting build */
/*extern*/XRSTATICLIGHTING_API FDebugLightingOutput GDebugStaticLightingInfo;
