...
struct FArmyColorAdjustmentParameters
{
	...
	/** Constructor */
	 FArmyColorAdjustmentParameters()
...
*/
struct FArmyTextureBuildSettings
{
	/** Color adjustment parameters. */
	 FArmyColorAdjustmentParameters ColorAdjustment;
...
class FArmyTextureCompressorModule :public IModuleInterface
{
public:
	virtual void StartupModule()override;
	virtual void ShutdownModule()override;

	ARMYTEXTURECOMPRESSOR_API static FArmyTextureCompressorModule& Get();
	virtual bool BuildTexture(const TArray<struct FImage>& SourceMips,
		const TArray<struct FImage>& AssociatedNormalSourceMips,
		const FArmyTextureBuildSettings& BuildSettings,
		TArray< FArmyCompressedImage2D>& OutTextureMips);

private:
	bool CompressMipChain(
		const TArray<FImage>& MipChain,
		const FArmyTextureBuildSettings& Settings,
		TArray< FArmyCompressedImage2D>& OutMips
	);
#if PLATFORM_WINDOWS
	// Handle to the nvtt dll
	void* nvTextureToolsHandle;
#endif	
	class IArmyTextureFormat* TextureFormat;
	bool BuildTextureMips(const TArray<FImage>& InSourceMips, const FArmyTextureBuildSettings& BuildSettings, TArray<FImage>& OutMipChain);
	static FArmyTextureCompressorModule* Singleton;
