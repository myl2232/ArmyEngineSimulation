...
FArmyActorPositionTraceResult FArmyActorPositioning::TraceWorldForPositionWithDefault(const FArmyViewportCursorLocation& Cursor, const FSceneView& View, const TArray<AActor*>* IgnoreActors)
{
	FArmyActorPositionTraceResult Results = TraceWorldForPosition(Cursor, View, IgnoreActors);
	if (Results.State == FArmyActorPositionTraceResult::Failed)
	{
		Results.State = FArmyActorPositionTraceResult::Default;

		// And put it in front of the camera
		const float DistanceMultiplier = 2000.f;// (Cursor.GetViewportType() == LVT_Perspective) ? GetDefault<ULevelEditorViewportSettings>()->BackgroundDropDistance : 0.0f;
		Results.Location = Cursor.GetOrigin() + Cursor.GetDirection() * DistanceMultiplier;
	}
	return Results;
}

FArmyActorPositionTraceResult FArmyActorPositioning::TraceWorldForPosition(const FArmyViewportCursorLocation& Cursor, const FSceneView& View, const TArray<AActor*>* IgnoreActors)
{
	const auto* ViewportClient = Cursor.GetViewportClient();

	FArmyActorPositionTraceResult Results;
	...
	if (ViewportType == EArmyLevelViewportType::LVT_OrthoXY || ViewportType == EArmyLevelViewportType::LVT_OrthoXZ || ViewportType == EArmyLevelViewportType::LVT_OrthoYZ ||
		ViewportType == EArmyLevelViewportType::LVT_OrthoNegativeXY || ViewportType == EArmyLevelViewportType::LVT_OrthoNegativeXZ || ViewportType == EArmyLevelViewportType::LVT_OrthoNegativeYZ)
	{
...
bool IsHitIgnored(const FHitResult& InHit, const FSceneView& InSceneView)
{
	...
	{				
		if (PrimitiveComponent && PrimitiveComponent->SceneProxy)
		{
			...
			//bIsRenderedOnScreen = ViewRelevance.bDrawRelevance || (PrimitiveComponent->IsA(UArmyModelComponent::StaticClass()) && InSceneView.Family->EngineShowFlags.BSP);
		}
	}
...
FArmyActorPositionTraceResult FArmyActorPositioning::TraceWorldForPosition(const UWorld& InWorld, const FSceneView& InSceneView, const FVector& RayStart, const FVector& RayEnd, const TArray<AActor*>* IgnoreActors)
{
	...
	if ( InWorld.LineTraceMultiByObjectType(Hits, RayStart, RayEnd, FCollisionObjectQueryParams(FCollisionObjectQueryParams::InitType::AllObjects), Param) )
	{
		...
		for (const auto& Hit : Hits)
		{
			...
			if (DistanceToHitSqr < ClosestHitDistanceSqr)
			{
				...
				Results.State = FArmyActorPositionTraceResult::HitSuccess;
...
// FTransform FArmyActorPositioning::GetCurrentViewportPlacementTransform(const AActor& Actor, bool bSnap)
// {
...
// }
// 
// FVector FArmyActorPositioning::GetActorPositionInFrontOfCamera(const AActor& InActor, const FVector& InCameraOrigin, const FVector& InCameraDirection)
// {
...
// }
// 
// FTransform FArmyActorPositioning::GetSurfaceAlignedTransform(const FPositioningData& Data)
// {
...
// }
// 
// FTransform FArmyActorPositioning::GetSnappedSurfaceAlignedTransform(const FSnappedPositioningData& Data)
// {
