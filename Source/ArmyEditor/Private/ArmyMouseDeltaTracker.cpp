...
#define LOCTEXT_NAMESPACE "ArmyMouseDeltaTracker"
...
FArmyMouseDeltaTracker::FArmyMouseDeltaTracker()
...
*/
void FArmyMouseDeltaTracker::DetermineCurrentAxis(UArmyEditorViewportClient* InViewportClient)
{
	return;
...
*/
void FArmyMouseDeltaTracker::StartTracking(UArmyEditorViewportClient* InViewportClient, const int32 InX, const int32 InY, const FArmyInputEventState& InInputState, bool bNudge, bool bResetDragToolState)
{
	...
	if (InViewportClient->Widget && InViewportClient->GetCurrentWidgetAxis() == EAxisList::None)
	{
		...
		if (HitProxy && HitProxy->IsA(HArmyWidgetAxis::StaticGetType()))
		{
			EAxisList::Type ProxyAxis = ((HArmyWidgetAxis*)HitProxy)->Axis;
	...
	if (InViewportClient->Widget)
	{
		...
		if (InViewportClient->GetWidgetMode() == FArmyWidget::WM_Rotate)
		{
	...
	//StartWorld = View->PixelToWorld(InX, InY, 0);
...
*/
bool FArmyMouseDeltaTracker::EndTracking(UArmyEditorViewportClient* InViewportClient)
{
	DetermineCurrentAxis(InViewportClient);
...
void FArmyMouseDeltaTracker::ConditionalBeginUsingDragTool(UArmyEditorViewportClient* InViewportClient)
{
	const bool LeftMouseButtonDown = InViewportClient->Viewport->KeyState(EKeys::LeftMouseButton);
	const bool RightMouseButtonDown = InViewportClient->Viewport->KeyState(EKeys::RightMouseButton);
	const bool MiddleMouseButtonDown = InViewportClient->Viewport->KeyState(EKeys::MiddleMouseButton);
	...
	if (bEnoughMouseMovement)
	{
		...
		if (bCanDrag && !bHasAttemptedDragTool)
		{
			...
			if (InViewportClient->IsOrtho())
			{
				...
				{
					if (InViewportClient->GetViewportType() != EArmyLevelViewportType::LVT_OnlyCanvas)
					{
						DragTool = InViewportClient->MakeDragTool(EArmyDragTool::BoxSelect);
					}
				}
			...
			else
			{
				...
				{
					DragTool = InViewportClient->MakeDragTool(EArmyDragTool::FrustumSelect);
				}
			}

			if (DragTool.IsValid())
			{
				DragTool->StartDrag(InViewportClient, GArmyEditor->ClickLocation, FVector2D(StartScreen));
			}
		}
...
*/
void FArmyMouseDeltaTracker::AddDelta(UArmyEditorViewportClient* InViewportClient, FKey InKey, const int32 InDelta, bool InNudge)
{
	const bool LeftMouseButtonDown = InViewportClient->Viewport->KeyState(EKeys::LeftMouseButton);
	const bool RightMouseButtonDown = InViewportClient->Viewport->KeyState(EKeys::RightMouseButton);
	const bool MiddleMouseButtonDown = InViewportClient->Viewport->KeyState(EKeys::MiddleMouseButton);
	...
	if (InViewportClient->GetCurrentWidgetAxis() != EAxisList::None)
	{
		...
		FArmyWidget::EWidgetMode WidgetMode = InViewportClient->GetWidgetMode();
		bool bIsRotation = (WidgetMode == FArmyWidget::WM_Rotate)
			|| ((WidgetMode == FArmyWidget::WM_TranslateRotateZ) && (InViewportClient->GetCurrentWidgetAxis() == EAxisList::ZRotation))
			|| ((WidgetMode == FArmyWidget::WM_2D) && (InViewportClient->GetCurrentWidgetAxis() == EAxisList::Rotate2D));
		if (bIsRotation)
		{
			Wk *= 0.3f;// GetDefault<ULevelEditorViewportSettings>()->MouseSensitivty;
		}
		else if (WidgetMode == FArmyWidget::WM_Scale /*&& !GEditor->UsePercentageBasedScaling()*/)
		{
		...
		//else if (InViewportClient->IsUsingAbsoluteTranslation())
		{
			...
			FSceneView* View = NULL;// = InViewportClient->CalcSceneView(&ViewFamily);
	...
	//EndWorld = View->PixelToWorld(End.X, End.Y, 0);
...
*/
const FVector FArmyMouseDeltaTracker::GetRawDelta() const
{
	return RawDelta;
}
...
*/
const FVector FArmyMouseDeltaTracker::GetDelta(bool bWorld) const
{
// 	if (bWorld)
// 	{
// 		return FVector(EndWorld - StartWorld);
// 	}
// 	else
// 	{
		return FVector(End - Start);
	//}
}
...
*/
const FVector FArmyMouseDeltaTracker::GetDeltaSnapped() const
{
...
*/
const FVector FArmyMouseDeltaTracker::GetAbsoluteDelta() const
{
...
*/
const FVector FArmyMouseDeltaTracker::GetAbsoluteDeltaSnapped() const
{
...
*/
const FVector FArmyMouseDeltaTracker::GetScreenDelta() const
{
...
*/
void FArmyMouseDeltaTracker::ConvertMovementDeltaToDragRot(UArmyEditorViewportClient* InViewportClient, FVector& InOutDragDelta, FVector& OutDrag, FRotator& OutRotation, FVector& OutScale) const
{
...
*/
void FArmyMouseDeltaTracker::AbsoluteTranslationConvertMouseToDragRot(FSceneView* InView, UArmyEditorViewportClient* InViewportClient, FVector& OutDrag, FRotator& OutRotation, FVector& OutScale) const
{
	OutDrag = FVector::ZeroVector;
	OutRotation = FRotator::ZeroRotator;
	OutScale = FVector::ZeroVector;
...
*/
void FArmyMouseDeltaTracker::ReduceBy(const FVector& In)
{
	End -= In;
	EndSnapped -= In;
	ReductionAmount += In;
...
*/
bool FArmyMouseDeltaTracker::UsingDragTool() const
{
...
*/
void FArmyMouseDeltaTracker::Render3DDragTool(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
	//if (DragTool.IsValid())
	//{
...
* Renders the drag tool.  Does nothing if no drag tool exists.
*/
void FArmyMouseDeltaTracker::RenderDragTool(const FSceneView* View, FCanvas* Canvas)
{
 	if (DragTool.IsValid())
 	{
...
const FVector FArmyMouseDeltaTracker::GetDragStartPos() const
{
	return Start;
}


const FVector FArmyMouseDeltaTracker::GetDragEndPos() const
{
	return End;
}

const bool FArmyMouseDeltaTracker::GetUsedDragModifier() const
{
	return bUsedDragModifier;
}

void FArmyMouseDeltaTracker::ResetUsedDragModifier()
{
	bUsedDragModifier = false;
}
