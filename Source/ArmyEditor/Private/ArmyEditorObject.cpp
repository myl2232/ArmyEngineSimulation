		...
		else if (GetBEGIN(&Str, TEXT("Object")))
		{
			...
			else
			{
				...
				// however that is fine since we will have been editing the CDO anyways
 				if (true)//!FBlueprintEditorUtils::IsAnonymousBlueprintClass(SubobjectOuter->GetClass()))
 				{
...
const TCHAR* ImportObjectProperties(FArmyImportObjectParams& InParams)
{
	...
	if (InParams.SubobjectOuter != NULL)
	{
		...
		if (InParams.bShouldCallEditChange)
		{
			...
			AActor* SubobjectOuterActor = Cast<AActor>(InParams.SubobjectOuter);
			if (SubobjectOuterActor)
			{
				SubobjectOuterActor->ReregisterAllComponents();
				SubobjectOuterActor->RerunConstructionScripts();
			}
		}
...
{
	FArmyImportObjectParams Params;
	{
