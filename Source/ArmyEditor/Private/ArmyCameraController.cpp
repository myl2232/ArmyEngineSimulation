...
/** Constructor */
FArmyEditorCameraController::FArmyEditorCameraController()
	: Config(),
 ...
 */
void FArmyEditorCameraController::UpdateSimulation(
	const FArmyCameraControllerUserImpulseData& UserImpulseData,
...
bool FArmyEditorCameraController::IsRotating (void) const
{
 ...
 */
void FArmyEditorCameraController::ApplyImpulseDeadZone( const FArmyCameraControllerUserImpulseData& InUserImpulse,
													FArmyCameraControllerUserImpulseData& OutUserImpulse,
 ...
 * @param	InOutCameraPosition		[in, out] Camera position
 */
void FArmyEditorCameraController::UpdatePosition( const FArmyCameraControllerUserImpulseData& UserImpulse, const float DeltaTime, const float MovementSpeedScale, const FVector& CameraEuler, FVector& InOutCameraPosition )
{
 ...
 */
void FArmyEditorCameraController::UpdateRotation( const FArmyCameraControllerUserImpulseData& UserImpulse, const float DeltaTime, FVector &InOutCameraEuler )
{
	...
	for( int32 CurRotationAxis = 0; CurRotationAxis < 3; ++CurRotationAxis )
	{
		...
		if( CurRotationAxis == 1 )		// 1 == pitch
		{
			...
			//allow for unlocked pitch constraints while in matinee
 			if (Config.bLockedPitch /*|| !GArmyLevelEditorModeTools().GetActiveMode(FBuiltinEditorModes::EM_InterpEdit)*/) 
 			{
 				// Clamp the angle.
 ...
 * @param	InOutCameraFOV	[in, out] Camera field of view
 */
void FArmyEditorCameraController::UpdateFOV( const FArmyCameraControllerUserImpulseData& UserImpulse, const float DeltaTime, float& InOutCameraFOV )
{
	// Compute acceleration
 ...
 * @param	InOutCameraFOV				[in, out] Camera field of view
 */
void FArmyEditorCameraController::ApplyRecoil( const float DeltaTime, const bool bAllowRecoilIfNoImpulse, bool bAnyUserImpulse, float& InOutCameraFOV )
{
