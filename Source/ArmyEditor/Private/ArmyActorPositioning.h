...
class AActor;
class FArmyLevelEditorViewportClient;
class FSceneView;
//class UActorFactory;
struct FArmyViewportCursorLocation;

// /** Positioning data struct */
// struct FArmyPositioningData
// {
// 	FArmyPositioningData(const FVector& InSurfaceLocation, const FVector& InSurfaceNormal)
...
// 		bAlignRotation = false;//Settings.bEnabled && Settings.bSnapRotation;
// 	}
// 
...
// 	FArmyPositioningData& UsePlacementExtent(const FVector& InPlacementExtent) { PlacementExtent = InPlacementExtent; return *this; }
...
// 	FArmyPositioningData& UseStartTransform(const FTransform& InStartTransform) { StartTransform = InStartTransform; return *this; }
// 	FTransform StartTransform;
// 
// 	/** A factory to use for the alignment. Factories define the alignment routine and spawn offset amounts if necessary. */
// 	FArmyPositioningData& UseFactory(const UActorFactory* InActorFactory) { ActorFactory = InActorFactory; return *this; }
// 	const UActorFactory* ActorFactory;
// 
// 	/** Whether to align to the surface normal, or just snap to its position */
// 	FArmyPositioningData& AlignToSurfaceRotation(bool bInAlignRotation) { bAlignRotation = bInAlignRotation; return *this; }
// 	bool bAlignRotation;
// };
// 
// /** Structure used for positioning actors with snapping */
// struct FArmySnappedPositioningData : FArmyPositioningData
// {
// 	FArmySnappedPositioningData(FLevelEditorViewportClient* InLevelViewportClient, const FVector& InSurfaceLocation, const FVector& InSurfaceNormal)
...
// 	FArmySnappedPositioningData& DrawSnapHelpers(bool bInDrawSnapHelpers) { bDrawSnapHelpers = bInDrawSnapHelpers; return *this; }
// 	bool bDrawSnapHelpers;
// 
// 	/** Mask these construction helpers to return the correct type */
// 	FArmySnappedPositioningData& UsePlacementExtent(const FVector& InPlacementExtent) { PlacementExtent = InPlacementExtent; return *this; }
// 	FArmySnappedPositioningData& UseStartTransform(const FTransform& InStartTransform) { StartTransform = InStartTransform; return *this; }
// 	FArmySnappedPositioningData& UseFactory(const UActorFactory* InActorFactory) { ActorFactory = InActorFactory; return *this; }
// 	FArmySnappedPositioningData& AlignToSurfaceRotation(bool bInAlignRotation) { bAlignRotation = bInAlignRotation; return *this; }
// };

struct FArmyActorPositionTraceResult
{
	...
	/** Constructor */
	FArmyActorPositionTraceResult() : State(Failed), Location(0.f), SurfaceNormal(0.f, 0.f, 1.f) {}
...
struct FArmyActorPositioning
{
	...
	*/
	static ARMYEDITOR_API FArmyActorPositionTraceResult TraceWorldForPositionWithDefault(const FArmyViewportCursorLocation& Cursor, const FSceneView& View, const TArray<AActor*>* IgnoreActors = nullptr);
	...
	*/
	static ARMYEDITOR_API FArmyActorPositionTraceResult TraceWorldForPosition(const FArmyViewportCursorLocation& Cursor, const FSceneView& View, const TArray<AActor*>* IgnoreActors = nullptr);
	 ...
	 *	@return	Result structure containing the location and normal of a trace hit, or empty on failure
	*/
	static ARMYEDITOR_API FArmyActorPositionTraceResult TraceWorldForPosition(const UWorld& InWorld, const FSceneView& InSceneView, const FVector& RayStart, const FVector& RayEnd, const TArray<AActor*>* IgnoreActors = nullptr);
	...
	*/
	//static ARMYEDITOR_API FTransform GetCurrentViewportPlacementTransform(const AActor& Actor, bool bSnap = true);
	 ...
	 *	@return	The expected transform for the specified actor
	*/
	//static ARMYEDITOR_API FVector GetActorPositionInFrontOfCamera(const AActor& InActor, const FVector& InCameraOrigin, const FVector& InCameraDirection);
	 ...
	 *	@return	The expected transform for the specified actor
	 */
	//static ARMYEDITOR_API FTransform GetSnappedSurfaceAlignedTransform(const FArmySnappedPositioningData& PositioningData);
	 ...
	 *	@return	The expected transform for the specified actor
	 */
	//static ARMYEDITOR_API FTransform GetSurfaceAlignedTransform(const FArmyPositioningData& PositioningData);
};
