...
static const FRotator GSNAPROTATION = FRotator(1, 1, 1);
static const float GSNAPSCALE = 0.25f;

/** Coordinate system identifiers. */
enum EArmyCoordSystem
{
	ArmyCOORD_None = -1,
	ArmyCOORD_World,
	ArmyCOORD_Local,
...
class FArmyWidget : public FGCObject
{
public:
	enum EWidgetMode
	{
	...
	*/
	ARMYEDITOR_API void SetUsesEditorModeTools(class FArmyEditorModeTools* InEditorModeTools);
	...
	*/
	...
	void Render(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient);

	// @param ViewportClient must not be 0
	//void RenderGrid(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient);
	...
	*/
	ARMYEDITOR_API void Render_Translate(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget);

	/**
	* Draws the rotation widget.
	*/
	ARMYEDITOR_API void Render_Rotate(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget);

	/**
	* Draws the scaling widget.
	*/
	void Render_Scale(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget);

	/**
	* Draws the Translate & Rotate Z widget.
	*/
	//void Render_TranslateRotateZ(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget);

	/**
	* Draws the combined 2D widget.
	*/
	//void Render_2D(const FSceneView* View, FPrimitiveDrawInterface* PDI, UArmyEditorViewportClient* ViewportClient, const FVector& InLocation, bool bDrawWidget);

	/**
	* Converts mouse movement on the screen to widget axis movement/rotation.
	*/
	void ConvertMouseMovementToAxisMovement(UArmyEditorViewportClient* InViewportClient, bool bInUsedDragModifier, FVector& InDiff, FVector& OutDrag, FRotator& OutRotation, FVector& OutScale);

	/**
	* Absolute Translation conversion from mouse movement on the screen to widget axis movement/rotation.
	*/
	void AbsoluteTranslationConvertMouseMovementToAxisMovement(FSceneView* InView, UArmyEditorViewportClient* InViewportClient, const FVector& InLocation, const FVector2D& InMousePosition, FVector& OutDrag, FRotator& OutRotation, FVector& OutScale);
	...
	*/
	...
	static bool AllowsAbsoluteTranslationMovement(FArmyWidget::EWidgetMode WidgetMode);
	...
	*/
	...
	ARMYEDITOR_API void UpdateDeltaRotation();
...
private:
	...
	struct FArmyAbsoluteMovementParams
	{
	...
	struct FArmyThickArcParams
	{
		FArmyThickArcParams(FPrimitiveDrawInterface* InPDI, const FVector& InPosition, UMaterialInterface* InMaterial, const float InInnerRadius, const float InOuterRadius)
			: Position(InPosition)
			, PDI(InPDI)
			, Material(InMaterial)
	...
	*/
	FVector GetAbsoluteTranslationDelta(const FArmyAbsoluteMovementParams& InParams);
	/**
	...
	*/
	void DrawThickArc(const FArmyThickArcParams& InParams, const FVector& Axis0, const FVector& Axis1, const float InStartAngle, const float InEndAngle, const FColor& InColor, const FVector& InDirectionToWidget, bool bIsOrtho);
	...
	*/
	...
	//*/
	//uint32 GetDominantAxisIndex(const FVector& InDiff, UArmyEditorViewportClient* ViewportClient) const;
	...
	class FArmyEditorModeTools* EditorModeTools;
	...
	*/
	...
	EArmyCoordSystem CustomCoordSystemSpace;
...
* Widget hit proxy.
*/
struct HArmyWidgetAxis : public HHitProxy
{
	...
	HArmyWidgetAxis(EAxisList::Type InAxis, bool InbDisabled = false) :
		HHitProxy(HPP_UI),
		Axis(InAxis),
