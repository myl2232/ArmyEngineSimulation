...
public:
    ...
    void PresentModalDialog(const FString& InMessage, FSimpleDelegate OnConfirm, bool bCloseImmediately = true, const FString& InTitle = TEXT("提醒"));//刘克祥 默认显示“提醒”
    ...
    /**
     * 显示提示消息
     * @param MessageType - EArmyMessageType - 消息类型
     * @param Message - const FString & - 消息内容
     */
	void ShowMessage(enum EArmyMessageType MessageType, const FString& Message);

    // zengy added
    void ShowMessageWithoutTimeout(EArmyMessageType MessageType, const FString& Message);
    void HideMessage();

	/** @刘克祥 显示屏幕底部信息，通常为“按F键交互”*/
...
private:
	...
	/** @刘克祥 交互信息提示（“按F键交互”），位于屏幕中底部 */
	TSharedPtr<SBorder> InteractiveMessageWidget;

	/** @刘克祥 窗口整体切换，用于设计模式和VR模式，其中索引0位设计模式的Widget，1为Viewport全屏VR模式 */
