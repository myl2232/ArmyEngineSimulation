...
FString FResTools::GetAutoCadDir(bool _Test /*= false*/)
{
	return FPaths::ProjectContentDir() + TEXT("ZKCache/AutoCad/");
}

FString FResTools::GetComponentDir(bool _Test /*= false*/)
{
...
FString FResTools::GetMD5FromFilePath(FString _FilePath)
{
	...
	if (pos != -1)
	{
		_FilePath = _FilePath.RightChop(pos + 1);
...
bool FResTools::ArmyOpenDirectoryDialog(const FString& DialogTitle, const FString& DefaultPath, FString& OutFolderName)
{
...
bool FResTools::ArmyOpenFileDialog(const FString& DialogTitle, const FString& DefaultPath, TArray<FString>& OutFileNames, FString _FileType, bool _bMulty)
{
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	if (DesktopPlatform)
	{
