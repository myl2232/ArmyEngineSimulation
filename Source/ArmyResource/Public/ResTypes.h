...
struct FArmyParameter
{
	...
	void OverrideValue(FArmyParameter& _Src)
	{
	...
	FArmyParameter(FString _DisplayName, FString _DisplayNameEN, FString _ParaName, float _MinValue, float _MaxValue, float _Value = 0.f)
	{
		ID = 0;
	...
	FArmyParameter(FString _DisplayName, FString _DisplayNameEN, FString _ParaName, FVector _Color = FVector::ZeroVector)
	{
		ID = 0;
		ParaDisplayName = _DisplayName;
		ParaDisplayNameEN = _DisplayNameEN;
	...
	FArmyParameter(FString _DisplayName, FString _DisplayNameEN, FString _ParaName, bool _bCheck)
	{
		ID = 0;
		ParaDisplayName = _DisplayName;
		ParaDisplayNameEN = _DisplayNameEN;
...
struct FModelMaterialData
{
	...
	FModelMaterialData(FString _SlotName, FCommonMaterialData::EMaterialType _MaterialType, UMaterialInstanceDynamic* _DIM, TArray<FArmyParameter>& _ParaList, bool _bReplaceable = false)
	...
	void ResetToServer()
	{
		...
		{
			It.ResetToServer();
			if (It.VarType == FArmyParameter::EVarType::E_Float)
			{
				DIM->SetScalarParameterValue(FName(*It.ParaName), It.Value);
			}
			else if (It.VarType == FArmyParameter::EVarType::E_Vec3)
			{
