 ...
 * 
 */
class ARMYRESOURCE_API FResTools
{
public:
	FResTools();
	...
	static FString GetAutoCadDir(bool _Test = false);
	static FString GetComponentDir(bool _Test = false);
	//  ../../../Army/Game/ZKCache/LR/LR_0.pak  =====>>  LR_0
	...
	bool ArmyOpenDirectoryDialog(const FString& DialogTitle, const FString& DefaultPath, FString& OutFolderName);
	bool ArmyOpenFileDialog(const FString& DialogTitle, const FString& DefaultPath, TArray<FString>& OutFileNames, FString _FileType, bool _bMulty);
	...
	//光源组件通用Get函数
	static bool GetLightComponentCastShadow(bool _bPointLight, AActor* _Actor);
	static float GetLightComponentIntensity(bool _bPointLight, AActor* _Actor);
	static FLinearColor GetLightComponentColor(bool _bPointLight, AActor* _Actor);
	static float GetLightComponentRadius(bool _bPointLight, AActor* _Actor);
	//射灯组件Get函数
	static float GetSpotLightComponentRadius(AActor* _Actor);
	static float GetSpotLightComponentInnerAngle(AActor* _Actor);
	static float GetSpotLightComponentOutterAngle(AActor* _Actor);
	static UTextureLightProfile* GetSpotLightComponentIES(AActor* _Actor);

	//光源组件通用Set函数
	static void SetLightComponentCastShadow(bool _bPointLight, AActor* _Actor, bool _CastShadow);
	static void SetLightComponentIntensity(bool _bPointLight, AActor* _Actor, float _Intensity);
	static void SetLightComponentColor(bool _bPointLight, AActor* _Actor, FLinearColor _Color);
	static void SetLightComponentRadius(bool _bPointLight, AActor* _Actor, float _Radius);
	//射灯组件Set函数
	static void SetSpotLightComponentRadius(AActor* _Actor, float _Raidus);
	static void SetSpotLightComponentInnerAngle(AActor* _Actor, float _InnerAngle);
	static void SetSpotLightComponentOutterAngle(AActor* _Actor, float _OutterAngle);
	static void SetSpotLightComponentIES(AActor* _Actor, UTextureLightProfile* _IES);
};
