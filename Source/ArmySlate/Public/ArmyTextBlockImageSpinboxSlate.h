...
public:
	...
	SLATE_BEGIN_ARGS(STextBlockImageSpinbox<NumericType>)
		: _Style(&FCoreStyle::Get().GetWidgetStyle<FSpinBoxStyle>("SpinBoxStyle"))//样式
		, _Value(0)
		, _MinValue(0)
		, _MaxValue(10)
		, _MinChangedValue(0.1)//微调数值
		, _Delta(0)
		...
		//@  欧石楠
		, _SliderVisibility(EVisibility::Visible)
	{}
	...
	SLATE_STYLE_ARGUMENT(FSpinBoxStyle, Style)
		...
		/**@欧石楠 微调的数值*/
		SLATE_ATTRIBUTE(TOptional< NumericType >, MinChangedValue)
		...
		//@  欧石楠
		SLATE_ATTRIBUTE(EVisibility, SliderVisibility)
	...
	*/
	void Construct(const FArguments& InArgs)
	{
		...
		MinChangedValue = InArgs._MinChangedValue;//@欧石楠 微调数值
		...
		this->ChildSlot
			...
			[
				...
				.Font(InArgs._Font)
				.Text(this, &STextBlockImageSpinbox<NumericType>::GetValueAsText)
				.MinDesiredWidth(this, &STextBlockImageSpinbox<NumericType>::GetTextMinDesiredWidth)
		...
		+ SHorizontalBox::Slot()
			...
			.Text(this, &STextBlockImageSpinbox<NumericType>::GetValueAsText)
			.OnIsTypedCharValid(this, &STextBlockImageSpinbox<NumericType>::IsCharacterValid)
			.OnTextChanged(this, &STextBlockImageSpinbox<NumericType>::TextField_OnTextChanged)
			.OnTextCommitted(this, &STextBlockImageSpinbox<NumericType>::TextField_OnTextCommitted)
		...
		+ SHorizontalBox::Slot()
			...
			[
				...
				[
					SNew(SButton)
					.OnHovered(this, &STextBlockImageSpinbox::OnUpHover)
					.OnUnhovered(this, &STextBlockImageSpinbox::OnUpUnHover)
					.ButtonStyle(&FArmyStyle::Get().GetWidgetStyle<FButtonStyle>("Button.None"))
					.OnClicked(this, &STextBlockImageSpinbox::OnArrowClicked, true)
					[
						SAssignNew(UpImage, SImage)
						.Image(FArmyStyle::Get().GetBrush("Icon.Spinbox_UpNormalArrow"))
					]
				]
				...
				[
					SNew(SButton)
					.OnHovered(this, &STextBlockImageSpinbox::OnDownHover)
					.OnUnhovered(this, &STextBlockImageSpinbox::OnDownUnHover)
					.ButtonStyle(&FArmyStyle::Get().GetWidgetStyle<FButtonStyle>("Button.None"))
					.OnClicked(this, &STextBlockImageSpinbox::OnArrowClicked, false)
					[
						SAssignNew(DownImage, SImage)
						.Image(FArmyStyle::Get().GetBrush("Icon.Spinbox_DownNormalArrow"))
					]
				]
			]
		];
	...
	/**欧石楠 点击三角按钮改变数字*/
	FReply OnArrowClicked(bool bAdd)
	{
		NumericType NewValue;
		if (bAdd)
		{
			NewValue = RoundIfIntegerValue(ValueAttribute.Get() + MinChangedValue.Get().GetValue());
			NewValue = NewValue > GetMaxValue() ? GetMinValue() : NewValue;
		}
		else
		{
			NewValue = RoundIfIntegerValue(ValueAttribute.Get() - MinChangedValue.Get().GetValue());
			NewValue = NewValue < GetMinValue() ? GetMaxValue() : NewValue;
		}

		OnValueCommitted.ExecuteIfBound(RoundIfIntegerValue(NewValue), ETextCommit::OnEnter);

		return FReply::Handled();
	}

	/**欧石楠 Hover改变三角按钮的颜色*/
	TSharedPtr<SImage> UpImage;
	TSharedPtr<SImage> DownImage;
	void OnUpHover()
	{
		UpImage->SetImage(FArmyStyle::Get().GetBrush("Icon.Spinbox_UpSelectedArrow"));
	}
	void OnDownHover()
	{
		DownImage->SetImage(FArmyStyle::Get().GetBrush("Icon.Spinbox_DownSelectedArrow"));
	}
	void OnUpUnHover()
	{
		UpImage->SetImage(FArmyStyle::Get().GetBrush("Icon.Spinbox_UpNormalArrow"));
	}
	void OnDownUnHover()
	{
		DownImage->SetImage(FArmyStyle::Get().GetBrush("Icon.Spinbox_DownNormalArrow"));
	}

	//@  欧石楠
	EVisibility GetSliderVisibility() const { return SliderVisibility; }
	void SetSliderVisibility(EVisibility _NewVisibility) { SliderVisibility = _NewVisibility; }
	EVisibility SliderVisibility;
...
protected:
	...
	{
		/* @欧石楠 最小改变量小于1时，只显示一位小数*/
		if (MinChangedValue.Get().GetValue() < 1)
		{
			FString Value = Interface->ToString(ValueAttribute.Get());
			float tempValue = FCString::Atof(Value.GetCharArray().GetData());
			FString Result = FString::Printf(TEXT("%.2f"), tempValue);
			return Result;
		}
		else
...
private:
	...
	TAttribute< TOptional<NumericType> > MinChangedValue;//微调数值
