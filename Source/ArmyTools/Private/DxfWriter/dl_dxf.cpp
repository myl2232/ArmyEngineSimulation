...
#endif
#define  DOUBLE(x,y)  	dw.dxfReal((x), (y))
#define  INT(x,y)	 dw.dxfInt((x), (y));
#define  HEX(x,y)	 dw.dxfHex((x), (y));
#define  STRING(x,y)	 dw.dxfString((x), (y));
...
void DL_Dxf::addEllipse(DL_CreationInterface* creationInterface) {
	DL_EllipseData d(toReal(values[40]),
		toReal(values[41]),
		toReal(values[42]),
		toReal(values[11]),
		toReal(values[21]),
 ...
 */
DL_WriterA* DL_Dxf::out(const char* file, DL_Codes::version inVersion) {
    char* f = new char[strlen(file)+1];
    strcpy(f, file);
    this->version = inVersion;

    DL_WriterA* dw = new DL_WriterA(f, inVersion);
    if (dw->openFailed()) {
        delete dw;
        delete[] f;
        return NULL;
...
void DL_Dxf::writeEllipse(DL_WriterA& dw,
    ...
    if (version>DL_VERSION_R12) {
		...
		dw.coord(210, data.nx, data.ny, data.nz);
    }
}
...
/**
* Writes a MLeader entity to the file.
*
* @param dw DXF writer
* @param data Entity data from the file
* @param attrib Attributes
* @see writeVertex
*/
void DL_Dxf::writeMLeader(DL_WriterA& dw,
	const DL_DimMLeaderData&data,
	const DL_Attributes& attrib)
{
	dw.entity("MULTILEADER");
	HEX(330, 0x1F);
	STRING(100, "AcDbEntity");
	STRING(8, attrib.getLayer());
	STRING(6, attrib.getLineType());
	INT(92, 720);
	STRING(100, "AcDbMLeader");
	STRING(300, "CONTEXT_DATA{");
	//SCALE CONTEXT
	DOUBLE(40, 1.0);
	DOUBLE(10, data.baseX);
	DOUBLE(20, data.baseY);
	DOUBLE(30, 1.0);
	DOUBLE(41, data.fontHeight);
	DOUBLE(140, data.arrowSize);
	DOUBLE(145, 2.0);
	//FONT ATTACHED
	INT(174, 1);
	INT(175, 1);
	...
	INT(177, 0);
	INT(290, 1);
	std::string simSun = "{\\fFangSong|b0|i0|c134|p2;";
	simSun += data.context;
	simSun += "}";
	STRING(304, simSun.c_str());
	DOUBLE(11, 0.0);
	DOUBLE(21, 0.0);
	DOUBLE(31, 1.0);
	HEX(340, styleHandleStd);

	DOUBLE(11, 0.0);
	DOUBLE(21, 0.0);
	DOUBLE(31, 1.0);
	//font base point
	DOUBLE(12, data.fontX);
	DOUBLE(22, data.fontY);
	DOUBLE(32, 0.0);
	//font direction
	DOUBLE(13, data.fontdx);
	DOUBLE(23, data.fontdy);
	DOUBLE(33, 0.0);
	...
	INT(295, 0);
	INT(296, 0);
	DOUBLE(110, data.baseX);
	DOUBLE(120, data.baseY);
	DOUBLE(130, 0.0);
	DOUBLE(111, 1.0);
	DOUBLE(121, 0.0);
	DOUBLE(131, 0.0);
	...
	DOUBLE(40, 0.0);
	//LEADER_LINE
	STRING(304, "LEADER_LINE{");
	DOUBLE(10, data.startX);
	DOUBLE(20, data.startY);
	DOUBLE(30, 0.0);
	DOUBLE(10, data.midX);
	DOUBLE(20, data.midY);
	DOUBLE(30, 0.0);
	...
	INT(170, 1);
	INT(91, -1056964608);
	HEX(341, styleHandleStd);
	INT(171, -2);
	INT(290, 1);
	INT(291, 0);
	//Leader Handle
	HEX(342, leaderHandle);
	DOUBLE(41, 0.0);
	DOUBLE(42, 4.0);
	INT(172, 2);
	INT(343, 71);
...
void DL_Dxf::writeHatch2(DL_WriterA& dw,
    ...
    if (data.solid==false) {
        ...
        //dw.dxfInt(78, 0);
		if (!DL_Pattern::get()->writePatternByName(data.pattern,dw, data.scale))
		{
			dw.dxfInt(78, 1);
			dw.dxfReal(53, 45.0);
			dw.dxfReal(43, 0.0);
...
void DL_Dxf::tableStyle(DL_WriterA& dw,int num)
{
	dw.dxfString(0, "TABLE");
 ...
 * to make the file readable by Aut*cad.
 */
void DL_Dxf::writeStyle(DL_WriterA& dw, DL_StyleData&style) {
...
/**
* Writes a MLeader section. This section is needed in DL_VERSION_R13.
* Note that this method currently only writes a faked UCS section
* to make the file readable by Aut*cad.
*/
void DL_Dxf::writeMLeader(DL_WriterA& dw)
{
	...
	dw.dxfString(2, "ACAD");
	dw.dxfInt(70, 0);
	dw.dxfString(0, "APPID");
	dw.handle();
	dw.dxfInt(330, 9);
	dw.dxfString(100, "AcDbSymbolTableRecord");
	dw.dxfString(100, "AcDbRegAppTableRecord");
	dw.dxfString(2, "ACAD_MLEADERVER");
	dw.dxfInt(70, 0);
	dw.tableEnd();
}
...
void DL_Dxf::writeObjects(DL_WriterA& dw) {
    ...
    //dw.dxfHex(350, dw.getNextHandle()+0);        // 1A
	 ...
	 dw.dxfString(3, "ACAD_MLEADERSTYLE");       //7ffffffe
	 dw.dxfHex(350, 0x7ffffffe);
	...
	dw.dxfString(0, "DICTIONARY");
	dw.dxfHex(5, 0x7ffffffe);
	dw.dxfHex(330, 0xC);
	dw.dxfString(100, "AcDbDictionary");
	dw.dxfInt(281, 1);
	dw.dxfString(3, "Standard");
	dw.dxfHex(350, 0x7fffffff);
...
void DL_Dxf::writeObjectMLeaderStyle(DL_WriterA& dw,double fontSize)
{
	dw.dxfString(0, "MLEADERSTYLE");
	//MLeader handle 
	dw.dxfHex(5, 0x7fffffff);
	dw.dxfHex(330, 0x7ffffffe);
	dw.dxfString(100, "AcDbMLeaderStyle");
	dw.dxfInt(170, 2);
	dw.dxfInt(171, 1);
	dw.dxfInt(172, 0);
	//max line sections : 3
	...
	dw.dxfReal(43, 0.0);
	dw.dxfString(3, "Standard");
	//arrow pattern handle : circle  hacthed
	dw.dxfHex(341, leaderHandle);
	//arrow size: 
	dw.dxfReal(44, 4.0);
	dw.dxfString(300, "");
	//font style handle
	dw.dxfHex(342, styleHandleStd);
	dw.dxfInt(174, 1);
	dw.dxfInt(178, 1);
	dw.dxfInt(175, 1);
	dw.dxfInt(176, 0);
	dw.dxfInt(93, -1056964608);
	//font height: via param
	dw.dxfReal(45, fontSize);
	dw.dxfInt(292, 0);
	dw.dxfInt(297, 0);
	dw.dxfReal(46, 4.0);
	dw.dxfInt(94, -1056964608);
	dw.dxfReal(47, 1.0);
	dw.dxfReal(49, 1.0);
	dw.dxfReal(140, 1.0);
	dw.dxfInt(293, 1);
	...
	dw.dxfInt(296, 1);
	dw.dxfReal(143, 3.75);
	dw.dxfString(1001, "ACAD_MLEADERVER");
	dw.dxfInt(1070, 2);
}
...
DL_Pattern* DL_Pattern::get()
{
	static  std::shared_ptr<DL_Pattern> ptr = std::make_shared<DL_Pattern>();
	return ptr.get();
}

void  DL_Pattern::initializePatternGlobal()
{
	addBlock();
}

bool  DL_Pattern::writePatternByName(const std::string&patternName, const DL_WriterA&dw, double scale)
{
	if (!callbackPool.size())
		initializePatternGlobal();
	auto iter = callbackPool.find(patternName);
	if (iter == callbackPool.end())
		return false;
	iter->second(const_cast<DL_WriterA&>(dw), scale);
	return true;
}
// "BLOCK" => CAD
 
void	DL_Pattern::addBlock()
{
 
	PatternCallback BLOCK = [](DL_WriterA&dw,double scale){
		INT(78, 2);
		DOUBLE(53, 0.0);
		DOUBLE(43, 0.0);
		DOUBLE(44, 0.0);
		...
		DOUBLE(44, 0.0);
		DOUBLE(45, -6.985);
		DOUBLE(46, 0.0000000000000004);
		INT(79, 2);
		DOUBLE(49, 5.08);
		DOUBLE(49, -1.905);
	};
	std::string block = "BLOCK";
	callbackPool.insert(std::make_pair(std::move(block), BLOCK));


	PatternCallback AR_HBONE = [](DL_WriterA&dw, double scale)
	{
		INT(78, 2);
		DOUBLE(53, 45.0);
		DOUBLE(43, 0.0);
		DOUBLE(44, 0.0);
		DOUBLE(45, 0.0);
		DOUBLE(46, 5.65685424949238*scale);
		INT(79, 2);
		DOUBLE(49, 12.0*scale);
		DOUBLE(49, -4.0*scale);
		DOUBLE(53, 135.0);
		DOUBLE(43, 2.828427125*scale);
		DOUBLE(44, 2.828427125*scale);
		DOUBLE(45, 0.000000000004);
		DOUBLE(46, 5.656854249492379*scale);
		INT(79, 2);
		DOUBLE(49, 12.0);
		DOUBLE(49, -4.0);
	};
	block = "AR-HBONE";
	callbackPool.insert(std::make_pair(std::move(block), AR_HBONE));

	PatternCallback NET = [](DL_WriterA&dw, double scale) {
		INT(78, 2);
		DOUBLE(53, 0.0);
		DOUBLE(43, 0.0);
		DOUBLE(44, 0.0);
		DOUBLE(45, 0.0);
		DOUBLE(46, 12.5*scale);
		DOUBLE(79, 0);
		DOUBLE(53, 90.0);
		DOUBLE(43, 0.0);
		DOUBLE(44, 0.0);
		DOUBLE(45, -12.5*scale);
		DOUBLE(46, 0.0000000000000008);
		INT(79, 0);
	};

	block = "NET";
	callbackPool.insert(std::make_pair(std::move(block), NET));

	PatternCallback ANSI21 = [](DL_WriterA&dw, double scale) {
		INT(78, 1);
		DOUBLE(53, 45.0);
		DOUBLE(43, 0.0);
		DOUBLE(44, 0.0);
		DOUBLE(45, -8.838834764831844*scale);
		DOUBLE(46, 8.838834764831844*scale);
		INT(79,0)
	};

	block = "ANSI31";
	callbackPool.insert(std::make_pair(std::move(block), ANSI21));

	PatternCallback AR_B816 = [](DL_WriterA&dw, double scale) {
		INT(78, 2);
		DOUBLE(53, 0.0);
		DOUBLE(43, 0.0);
		DOUBLE(44, 0.0);
	...
	block = "AR-B816";
	callbackPool.insert(std::make_pair(std::move(block), AR_B816));
}
