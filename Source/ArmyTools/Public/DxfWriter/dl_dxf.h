...
public:
    ...
    void writeLeader(DL_WriterA& dw,
                     ...
                     const DL_Attributes& attrib);
	void writeMLeader(DL_WriterA& dw,
	...
	void tableStyle(DL_WriterA& dw,int num);
    void writeStyle(DL_WriterA& dw,DL_StyleData&style);
	...
	void writeObjectMLeaderStyle(DL_WriterA& dw, double fontSize);
...
private:
	...
	int libVersion;
	// architecture handle
	unsigned long  archHandle;
	unsigned long	 leaderHandle;
};

typedef std::function<void(DL_WriterA&,double scale)>PatternCallback;
class DL_Pattern
{
public:
	DL_Pattern() = default;
	~DL_Pattern() = default;
public:
	static DL_Pattern* get();
 
	void  initializePatternGlobal();
	
	bool  writePatternByName(const std::string&,const DL_WriterA&,double scale);


	std::unordered_map<std::string, PatternCallback> callbackPool;
private:
	// "BLOCK" => CAD
	void	addBlock();
	//TODO:
